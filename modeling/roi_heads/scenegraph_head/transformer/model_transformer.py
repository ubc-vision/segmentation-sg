...
import numpy as np
from ..motif.utils_motifs import cat
from ..motif.utils_motifs import obj_edge_vectors, to_onehot, encode_box_info
...
class TransformerContext(nn.Module):
    def __init__(self, config, obj_classes, rel_classes, in_channels):
        ...
        if self.cfg.MODEL.ROI_SCENEGRAPH_HEAD.USE_GT_BOX:
            self.mode = 'predcls' if self.cfg.MODEL.ROI_SCENEGRAPH_HEAD.USE_GT_OBJECT_LABEL else 'sgcls'
        else:
            self.mode = 'sgdet'
        ...
        self.dropout_rate = self.cfg.MODEL.ROI_SCENEGRAPH_HEAD.TRANSFORMER.DROPOUT_RATE   
        self.obj_layer = self.cfg.MODEL.ROI_SCENEGRAPH_HEAD.TRANSFORMER.OBJ_LAYER      
        self.edge_layer = self.cfg.MODEL.ROI_SCENEGRAPH_HEAD.TRANSFORMER.REL_LAYER        
        self.num_head = self.cfg.MODEL.ROI_SCENEGRAPH_HEAD.TRANSFORMER.NUM_HEAD         
        self.inner_dim = self.cfg.MODEL.ROI_SCENEGRAPH_HEAD.TRANSFORMER.INNER_DIM     
        self.k_dim = self.cfg.MODEL.ROI_SCENEGRAPH_HEAD.TRANSFORMER.KEY_DIM         
        self.v_dim = self.cfg.MODEL.ROI_SCENEGRAPH_HEAD.TRANSFORMER.VAL_DIM    
    ...
    def forward(self, roi_features, proposals, boxes, logger=None):
        # labels will be used in DecoderRNN during training
        use_gt_label = self.training or self.cfg.MODEL.ROI_SCENEGRAPH_HEAD.USE_GT_OBJECT_LABEL
        obj_labels = cat([proposal.pred_classes for proposal in proposals], dim=0) if use_gt_label else None
        ...
        pos_embed = self.bbox_embed(encode_box_info(boxes, img_sizes))
    ...
    def nms_per_cls(self, obj_dists, boxes_per_cls, num_objs):
        ...
        for i in range(len(num_objs)):
            ...
            #Last index for background
            out_dists_sampled[:, -1] = -1
