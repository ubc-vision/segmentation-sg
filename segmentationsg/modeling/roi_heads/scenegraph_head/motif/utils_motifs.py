...
def generate_attributes_target(attributes, device, max_num_attri, num_attri_cat):
        ...
        for idx in torch.nonzero(with_attri_idx, as_tuple=False).squeeze(1).tolist():
...
def sort_by_score(boxes, scores):
    """
...
def center_x(boxes):
    ...
    boxes = cat([p.tensor for p in boxes], dim=0)
    c_x = 0.5 * (boxes[:, 0] + boxes[:, 2])
    return c_x.view(-1)
    
def encode_box_info(boxes_list, img_sizes):
    """
    ...
    boxes_info = []
    for (boxes, img_size) in zip(boxes_list, img_sizes):

        wid = img_size[0]
        hei = img_size[1]
        wh = boxes.tensor[:, 2:] - boxes.tensor[:, :2] + 1.0
        xy = boxes.tensor[:, :2] + 0.5 * wh
        w, h = wh.split([1,1], dim=-1)
        x, y = xy.split([1,1], dim=-1)
        x1, y1, x2, y2 = boxes.tensor.split([1,1,1,1], dim=-1)
        assert wid * hei != 0
...
def obj_edge_vectors(names, wv_dir, wv_type='glove.6B', wv_dim=300):
    ...
    for i, token in enumerate(names):
        ...
        if wv_index is not None:
            ...
            logger.info("{} -> {} ".format(token, lw_token))
            wv_index = wv_dict.get(lw_token, None)
            if wv_index is not None:
                vectors[i] = wv_arr[wv_index]
            else:
                logger.warning("fail on {}".format(token))

    return vectors

def obj_edge_vectors_segmentation(names, wv_dir, wv_type='glove.6B', wv_dim=300):
    wv_dict, wv_arr, wv_size = load_word_vectors(wv_dir, wv_type, wv_dim)
